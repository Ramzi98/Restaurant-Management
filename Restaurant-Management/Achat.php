...
<!-------------------------------------->
    ...
    <div>
        <div class="container">
            <div class="row">
                <div class="col-md-12 text-center"><label class="col-form-label border rounded border-secondary shadow" style="padding: 10px;font-size: 21px;margin: 1px;background-color: rgb(231,231,231);color: rgb(0,0,0);padding-top: 3px;padding-bottom: 3px;">قائمة المشتريات<br>Purchases List<br></label></div>
            </div>
        </div>
    </div>
    <div class="col text-center" style="margin-top: 10px;"><button class="btn btn-primary" type="button"><i class="fa fa-chevron-left"></i></button><label class="border rounded border-primary" style="background-color: #ffffff;width: 30px;">1</label><button class="btn btn-primary" type="button"><i class="fa fa-chevron-right"></i></button></div>
    <div
        class="col">
 ...
 $('#image_form').submit(function(event){
  ...
  event.preventDefault();
    ...
    $.ajax({
     url:"Achat_action.php",
     method:"POST",
     data: {name:name,price:prix,date:date,id:id,action:action},
     success:function(data)
     {
      fetch_data();
      $('#image_form')[0].reset();
